...
import { setFilter } from "../../../reduxToolkit/cs/filter/FilterSliceCs";
...
const FilterAcTypeMenuCs = (props) => {
    ...
    const acTypeListStore = useSelector(state => state.allFiltersCs.filters.aircraftTypeList)
    const acTailListStore = useSelector(state => state.allFiltersCs.filters.aircraftTailList)
    ...
    useEffect(() => {
        ...
        setCheckedAcType(acTypeListStore)
        setCheckedAcTail(acTailListStore)
    }, [])
    ...
    const showAircraftTypeList = () => {
        return aircraftTypeList.map((acTypeItem, i) => {
            return (
                <div className="aircraft-type__list-wrapper" key={i}>
                    <div className="aircraft-type__group">
    ...
    const applyFilter = () => {
        dispatch(
            setFilter({
                aircraftTypeList: checkedAcType,
                aircraftTailList: checkedAcTail
            })
        )
        props.acTypeFilterRef.current.click()
    }
